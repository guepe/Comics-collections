...
class TestDedupEngine(TransactionCase):
    ...
    def test_conflit_certain_same_serie_tome(self):
        ...
        candidates = self.env["comic.work"]._find_duplicate_candidates(self.serie_a.id, 1, "Autre titre")
    ...
    def test_doublon_probable_same_serie_similar_title(self):
        ...
        candidates = self.env["comic.work"]._find_duplicate_candidates(self.serie_a.id, 99, "Landes perdues (Les)")
    ...
    def test_doublon_possible_similar_serie_and_title(self):
        ...
        self._make_work("La Magicienne trahie", self.serie_a, 1)
        # Different serie (similar name), same title → doublon_possible
        candidates = self.env["comic.work"]._find_duplicate_candidates(serie_c.id, 1, "La Magicienne trahie")
    ...
    def test_constrains_skip_with_context(self):
        ...
        w = (
            self.env["comic.work"]
            .with_context(skip_dedup_check=True)
            .create(
                {
...
class TestMerge(TransactionCase):
    ...
    def test_merge_transfers_auteur_lines_without_duplicates(self):
        ...
        shared_lines = target.auteur_line_ids.filtered(lambda line: line.partner_id == partner_shared)
