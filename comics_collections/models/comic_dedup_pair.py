...
class ComicDedupPair(models.Model):
    ...
    work_a_id = fields.Many2one(
        'comic.work', required=True, ondelete='cascade', string='Album A')
    work_b_id = fields.Many2one(
        'comic.work', required=True, ondelete='cascade', string='Album B')
    score = fields.Float(string='Score', digits=(4, 3))
    ...
    def action_scan_all(self):
        ...
        ignored_keys = {
            (min(p.work_a_id.id, p.work_b_id.id),
             max(p.work_a_id.id, p.work_b_id.id))
            for p in ignored
        }
        ...
        for i, wa in enumerate(work_list):
            for wb in work_list[i + 1:]:
                ...
                same_serie = bool(wa.serie_id and wb.serie_id
                                  and wa.serie_id.id == wb.serie_id.id)
                score, reason = _score_pair(
                    same_serie,
                    wa.tome, wa.titre_normalise or normalize_title(wa.titre_canonique),
                    wa.serie_id.name_normalise or '',
                    wb.tome, wb.titre_normalise or normalize_title(wb.titre_canonique),
        ...
        for vals in new_pairs:
            try:
                ...
                pass  # unique constraint: pair already exists (shouldn't happen after unlink)
