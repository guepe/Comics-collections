...
class ComicIsbn(models.Model):
    ...
    edition_id = fields.Many2one(
        'comic.edition', string='Édition', required=True, ondelete='cascade', index=True)
        ...
        for rec in self:
            if rec.isbn_13 and not self._validate_ean13(rec.isbn_13):
                raise ValidationError(
                    _('"%s" n\'est pas un EAN-13 valide.') % rec.isbn_13)
    ...
    def _check_isbn_10(self):
        for rec in self:
            if rec.isbn_10 and not self._validate_isbn10(rec.isbn_10):
                raise ValidationError(
                    _('"%s" n\'est pas un ISBN-10 valide.') % rec.isbn_10)
