...
class ComicSerieMissingWizard(models.TransientModel):
    ...
    def action_add_to_wishlist(self):
        ...
        if added:
            items = ''.join(
                f'<li>Tome {l.tome} — {l.name}</li>' for l in added
            )
            self.import_report = (
                f'<b>✅ {len(added)} tome(s) ajouté(s) à la wishlist :</b><ul>{items}</ul>'
            )
        else:
...
class ComicSerieMissingLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        'comic.serie.missing.wizard', required=True, ondelete='cascade')
