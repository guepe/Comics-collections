...
class ComicDatasourceSearchWizard(models.TransientModel):
    ...
    def action_search(self):
        ...
        self.env["comic.datasource.result.line"].create([dict(v, wizard_id=self.id) for v in lines])
        self.state = "results"
        return self._reopen()

    def _aggregated_to_line_vals(self, agg, isbn):
        data = agg.data
        auteurs_display = ", ".join(f"{a['name']} ({a.get('role', '')})" for a in data.get("auteurs", []))
        return {
            "selected": True,
    ...
    def _source_result_to_line_vals(self, result):
        auteurs_display = ", ".join(f"{a['name']} ({a.get('role', '')})" for a in (result.auteurs or []))
        data = result.to_dict()
        return {
            "selected": False,
    ...
    def _find_edition_by_isbn(self, isbn):
        ...
        isbn_rec = self.env["comic.isbn"].search(["|", ("isbn_13", "=", isbn), ("isbn_13", "=", clean)], limit=1)
        if isbn_rec:
    ...
    def _fetch_cover(self, url):
        ...
        try:
            ...
            _logger.debug("Failed to fetch cover from %s", url)
        return False
    ...
    def _parse_date_into_vals(vals, field, date_str):
        ...
        try:
            ...
            _logger.debug("Invalid date format for %s: %s", field, date_str)
...
class ComicDatasourceResultLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one("comic.datasource.search.wizard", required=True, ondelete="cascade")
