...
class ComicShopController(WebsiteSale):
    ...
    def product(self, product, category="", search="", **kwargs):
        ...
        if not request.website.is_public_user():
            in_library = bool(
                request.env["comic.customer.album"].search_count(
                    [
    ...
    def _shop_lookup_products(self, options, post, search, website):
        ...
        fuzzy_term, product_count, search_result = super()._shop_lookup_products(options, post, search, website)
        ...
        allowed_ids = set(request.env["comic.edition"].sudo().search(edition_domain).mapped("product_tmpl_id.id"))
    ...
    def shop_series(self, **kwargs):
        ...
        for serie in series.filtered(lambda s: not s.image_couverture):
            ...
            if first:
                cover_urls[serie.id] = f"/web/image/product.template/{first[0].product_tmpl_id.id}/image_512"
        return request.render(
            "comic_shop.shop_series_page",
            {
                "series": series,
    ...
    def shop_serie_detail(self, serie_id, **kwargs):
        ...
        return request.render(
            "comic_shop.shop_serie_detail_page",
            {
                "serie": serie,
                "editions": editions,
            },
    ...
    def shop_auteurs(self, **kwargs):
        lines = (
            request.env["comic.work.auteur.line"]
            .sudo()
            .search(
                [
                    ("work_id.edition_ids.product_tmpl_id", "!=", False),
        ...
        for data in sorted(auteur_map.values(), key=lambda d: (d["partner"].name or "").lower()):
            data["role_labels"] = ", ".join(_ROLE_LABELS.get(r, r) for r in sorted(data["roles"]))
            auteurs.append(data)
    ...
    def shop_auteur_detail(self, auteur_id, **kwargs):
        ...
        lines = lines.sorted(key=lambda line: (line.work_id.serie_id.name or "", line.work_id.tome or 0))
        by_role = {}
        for line in lines:
        ...
        return request.render(
            "comic_shop.shop_auteur_detail_page",
            {
                "auteur": auteur,
                "by_role": by_role,
    ...
    def shop_editeurs(self, **kwargs):
        ...
        for serie in series_with_products:
            ...
            editeur_map[eid]["nb_albums"] += len(serie.work_ids.edition_ids.filtered(lambda e: e.product_tmpl_id))
        editeurs = sorted(editeur_map.values(), key=lambda d: (d["editeur"].name or "").lower())
    ...
    def shop_editeur_detail(self, editeur_id, **kwargs):
        ...
        return request.render(
            "comic_shop.shop_editeur_detail_page",
            {
                "editeur": editeur,
                "series": series,
            },
