...
class ComicLibraryPortal(CustomerPortal):
    ...
    def my_library(self, filter="all", sort="date", view="kanban", **kw):
        ...
        all_records = (
            request.env["comic.customer.album"]
            .sudo()
            .search(
                [
        ...
        return request.render(
            "comic_shop.portal_library_index",
            {
                "records": records,
                "filter": filter,
                "sort": sort,
    ...
    def my_library_detail(self, cust_album_id, **kw):
        ...
        return request.render(
            "comic_shop.portal_library_detail",
            {
                "record": record,
                "page_name": "library",
            },
    ...
    def my_library_update(self, cust_album_id, **post):
        ...
        try:
            ...
            _logger.debug("Invalid note value: %s", post.get("note"))
    ...
    def my_library_add_search(self, **post):
        ...
        # Local DB — ISBN exact
        if isbn:
            isbn_rec = request.env["comic.isbn"].sudo().search([("isbn_13", "=", isbn)], limit=1)
            if isbn_rec:
        ...
        if not results and title_raw:
            works = (
                request.env["comic.work"]
                .sudo()
                .search([("titre_canonique", "ilike", title_raw)], limit=5, order="id desc")
            )
            for work in works:
                if work.edition_ids:
    ...
    def _edition_to_result(self, edition, is_local=True):
        ...
        auteurs = ", ".join(line.partner_id.name for line in work.auteur_line_ids if line.partner_id)
    ...
    def _create_edition_from_post(self, post):
        ...
        editeur = None
        if editeur_name:
            editeur = env["comic.editeur"].sudo().search([("name", "=", editeur_name)], limit=1)
            if not editeur:
        ...
        serie = None
        if serie_name:
            serie = env["comic.serie"].sudo().search([("name", "=", serie_name)], limit=1)
            if not serie:
        ...
        work = None
        if serie and tome:
            work = env["comic.work"].sudo().search([("serie_id", "=", serie.id), ("tome", "=", tome)], limit=1)
        if not work and serie:
            work = (
                env["comic.work"].sudo().search([("serie_id", "=", serie.id), ("titre_canonique", "=", title)], limit=1)
            )
        if not work:
        ...
        # Edition
        edition = env["comic.edition"].sudo().search([("work_id", "=", work.id)], limit=1)
        if not edition:
