...
SERIES = [
    {
        ...
        'albums': [
            ...
            (12, '2803606399',    'La cité du dieu perdu',            '1987-10-01', 'Jean Van Hamme',  'Grzegorz Rosinski', True,  'lu'),
            (15, '2803607549',    'Le maître des montagnes',          '1989-10-01', 'Jean Van Hamme',  'Grzegorz Rosinski', True,  'lu'),
            ...
            (26, '2803616653',    'Le Royaume sous le Sable',         '2001-11-01', 'Jean Van Hamme',  'Grzegorz Rosinski', True,  'lu'),
            ...
            (35, '9782803635481', 'Le feu écarlate',                  '2016-11-01', 'Xavier Dorison',  'Grzegorz Rosinski', False, 'non_lu'),
            ...
            (43, '9782808214865', 'La vengeance de la déesse Skædhi', '2025-11-01', 'Yann',            'Fred Vignaux',      False, 'non_lu'),
        ],
    },
    {
        ...
        'albums': [
            ...
            (4,  '2871291691',    'Sioban 4 - Kyle of Klanach',                '1998-10-01', 'Jean Dufaux', 'Grzegorz Rosinski', True, 'lu'),
            (5,  '2871296839',    'Les Chevaliers du Pardon 1 - Moriganes',    '2004-10-01', 'Jean Dufaux', 'Philippe Delaby',   True, 'lu'),
            (6,  '9782505004646', 'Les Chevaliers du Pardon 2 - Le Guinea Lord','2008-10-01', 'Jean Dufaux', 'Philippe Delaby',   True, 'lu'),
            (7,  '9782505013877', "Les Chevaliers du Pardon 3 - La Fée Sanctus",'2012-06-01', 'Jean Dufaux', 'Philippe Delaby',   True, 'lu'),
            (8,  '9782505019794', 'Les Chevaliers du Pardon 4 - Sill Valt',    '2014-11-01', 'Jean Dufaux', 'Philippe Delaby',   True, 'lu'),
            (9,  '9782505063506', 'Les Sorcières 1 - Tête noire',              '2015-10-01', 'Jean Dufaux', 'Béatrice Tillier',  True, 'lu'),
            (10, '9782505063995', 'Les Sorcières 2 - Inferno',                 '2019-01-01', 'Jean Dufaux', 'Béatrice Tillier',  True, 'lu'),
            (11, '9782505078197', 'Les Sorcières 3 - Regina Obscura',          '2023-03-01', 'Jean Dufaux', 'Béatrice Tillier',  True, 'lu'),
            ...
            (15, '9782505113607', 'Les Sudenne 3 - La Folie Seamus',           '2023-10-01', 'Jean Dufaux', 'Paul Teng',         False, 'non_lu'),
            (16, '9782505122739', "Les Sudenne 4 - Lady O'Mara",               '2024-11-01', 'Jean Dufaux', 'Paul Teng',         False, 'non_lu'),
        ],
    },
]
    ...
    for serie_data in SERIES:
        ...
        if gname:
            if gname not in genre_cache:
                genre_cache[gname] = odoo.get_or_create(
                    'comic.genre', [('name', '=', gname)], {'name': gname})
        ...
        editeur_id = False
        if ename:
            editeur_id = odoo.get_or_create(
                'comic.editeur', [('name', '=', ename)], {'name': ename})

        # Série
        serie_id = odoo.get_or_create(
            'comic.serie',
            [('name', '=', sname)],
            {'name': sname, 'type': serie_data['type'],
             'genre_id': genre_id, 'editeur_id': editeur_id},
        )
        stats['series'] += 1
        print(f"\n📚  Série : {sname}  (id={serie_id})")

        for (tome, isbn, titre, date_depot, scenariste, dessinateur,
             dans_collection, etat_lecture) in serie_data['albums']:

            # Déduplique : cherche un album existant pour ce tome
            existing = odoo.search('comic.album', [
                ('serie_id', '=', serie_id), ('tome', '=', tome)])
            if existing:
                album_id = existing[0]
            ...
            for role, aname in [('scenariste', scenariste), ('dessinateur', dessinateur)]:
                ...
                if not odoo.search('comic.album.auteur.line', [
                    ('album_id', '=', album_id),
                    ('partner_id', '=', pid),
                    ('role', '=', role),
                ]):
                    odoo.create('comic.album.auteur.line', {
                        'album_id': album_id, 'partner_id': pid, 'role': role})
            ...
            if publish and isbn:
                ...
                if data and not data[0].get('product_tmpl_id'):
                    ...
                    pvals = {
                        'name': pname, 'type': 'consu',
                        'comic_album_id': album_id,
                        'barcode': isbn, 'is_published': True,
                    }
                    if category_id:
