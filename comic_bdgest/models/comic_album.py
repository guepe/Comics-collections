...
class ComicEdition(models.Model):
    ...
    def action_batch_enrich_from_bdgest(self):
        ...
        if not editions_with_isbn:
            raise UserError(
                "Aucune des éditions sélectionnées n'a d'ISBN renseigné."
            )
    ...
    def _bdgest_apply_detail(self, detail, scraper):
        ...
        if isbn:
            ...
            ed_vals['url_club_be'] = f"https://www.librairieclub.be/c/search?filter=search({isbn_clean})&page=1&page_size=24&sort=RelevanceClub&sort_type=desc"
