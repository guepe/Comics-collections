...
class ComicBdgestEnrichWizard(models.TransientModel):
    ...
    result_ids = fields.One2many("comic.bdgest.result.line", "wizard_id", string="Résultats")
    ...
    def action_search(self):
        ...
        try:
            ...
            if self.search_type == "isbn":
                ...
                if not detail:
                    raise UserError(_("Aucun album trouvé pour cet ISBN sur BDGest."))
        ...
        if not raw:
            raise UserError(_("Aucun résultat trouvé. Essayez une autre recherche."))
    ...
    def action_import(self):
        ...
        if not selected:
            raise UserError(_("Sélectionnez un résultat à importer."))
        if len(selected) > 1:
            raise UserError(_("Sélectionnez un seul résultat."))
        line = selected[0]
...
class ComicBdgestResultLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one("comic.bdgest.enrich.wizard", ondelete="cascade", required=True)
